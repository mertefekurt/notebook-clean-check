...
from notebook_clean_check import __version__
from notebook_clean_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from notebook_clean_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='notebook-clean-check',
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='notebook JSON or exported notebook text',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"notebook-clean-check: error: {exc}", file=sys.stderr)
        return 1
